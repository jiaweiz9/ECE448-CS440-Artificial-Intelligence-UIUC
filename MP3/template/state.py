...
class EightPuzzleState(AbstractState):
    ...
    def get_neighbors(self):
        ...
        moves = ((1, 0), (0, -1), (-1, 0), (0, 1)) # moves for the empty tile
        for x, y in moves:
            if self.zero_loc[0] + x >= 0 and self.zero_loc[0] + x < 3 \
                and self.zero_loc[1] + y >= 0 and self.zero_loc[1] + y < 3:
                zero_loc_neighbor = [self.zero_loc[0] + x, self.zero_loc[1] + y]
                potential_nbr = copy.deepcopy(self.state)
                potential_nbr[self.zero_loc[0]][self.zero_loc[1]] = potential_nbr[zero_loc_neighbor[0]][zero_loc_neighbor[1]]
                potential_nbr[zero_loc_neighbor[0]][zero_loc_neighbor[1]] = 0

                new_state = EightPuzzleState(potential_nbr, self.goal, 
                                                dist_from_start=self.dist_from_start + 1, use_heuristic=self.use_heuristic, zero_loc=zero_loc_neighbor)
                nbr_states.append(new_state)
        return nbr_states
    ...
    def compute_heuristic(self):
        ...
        def find_loc_in_goal(number):
            for x in range(3):
                for y in range(3):
                    if number == self.goal[x][y]:
                        return x, y
            return []
        ...
        for i in range(3):
            for j in range(3):
                if self.state[i][j] != 0:
                    total += manhattan([i, j], find_loc_in_goal(self.state[i][j]))
        return total
