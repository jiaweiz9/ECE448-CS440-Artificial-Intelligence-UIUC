...
epsilon_for_pt = 1e-5
alpha = 1e-5   # exact setting seems to have little or no effect
...
def training(sentences):
	...
	hapax_word_dict = defaultdict(lambda: 0)
	for word, tag in hapax_word_tag:
		hapax_word_dict[tag] += 1

	print(hapax_word_dict)
	for tag in emit_prob.keys():
		if tag in hapax_word_dict.keys():
			emit_prob[tag]['UNSEEN'] += alpha * hapax_word_dict[tag] / numOfHapax
		else:
			emit_prob[tag]['UNSEEN'] += alpha * 1 / numOfHapax
...
def viterbi_stepforward(i, word, prev_prob, prev_predict_tag_seq, emit_prob, trans_prob):
	...
	for tag in tag_list:
		if i == 0:
			...
			#emit_prob_cur = emit_prob[tag][word] if word in emit_prob[tag].keys() else emit_prob[tag]['UNSEEN']
			beam_search_dict = dict(sorted(prev_prob.items(), key=lambda item: item[1], reverse=True)[0:5])
